# examples/run_all_group2_all_experiments.py
"""
总控脚本：批量跑 Group2（上层 BA + 下层规则派工）
...
本脚本行为：
  - 对每个算例 exp_name，调用 group2_two_level_rules.run_group2_for_experiment()
  - 每个算例：
      * 规则集：["fifo", "spt", "lpt", "srpt"]
      * 种子：0..9 共 10 个
"""
...
# ---------- 导入 Group2 函数 ----------
from group2_two_level_rules import run_group2_for_experiment
...
RULES = ["fifo", "spt", "lpt", "srpt"]   # Group2 要跑的规则集合
SEEDS = list(range(10))                  # 每个算例、每条规则 10 个种子
DEVICE = "cuda"
NUM_OUTER_EPISODES = 400                 # 上层 BA 的 outer episodes 数
...
# ============================================================
# main：遍历所有算例，调用 Group2
# ============================================================

def main():
    exps = build_experiment_list()
    print("[INFO] Experiments to run (Group2 - BA + Rule):")
    for e in exps:
    ...
    for e in exps:
        print("\n" + "=" * 80)
        print(
            f"[MASTER] Running Group2 for experiment {e.name} "
